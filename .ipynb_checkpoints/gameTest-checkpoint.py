...
class Player():
    ...
    def draw(self, win):
        ...
        if self.jump:
            ...
            if self.toLeft:
                win.blit(pygame.transform.flip(heroMovingJump[int(self.movJumpCount//10)], 1, 0),(self.x, self.y))    # walkCount changes the index so the images line up with the framerate //3 is integer division
                self.movJumpCount += 1
            else:
                win.blit(pygame.transform.flip(heroMovingJump[int(self.movJumpCount//10)], 0, 0),(self.x, self.y))
                self.movJumpCount += 1
