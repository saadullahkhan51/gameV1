...
unarmed_hero_sheet = pygame.image.load("Sprites\adventurer\adventurer-Sheet.png").convert() # 0-3 idle, 4-7 crouch, 8-13 run, 14-23 movingjump,
hand_combat_sheet = pygame.image.load("Sprites\adventurer\adventurer-hand-combat-Sheet.png").convert()
...
class Player():
    ...
    def draw(self, win):
        ...
        if not(self.attack):
            if not(self.idle):
                if self.left:
                    win.blit(walkLeft2[self.walkCount//3],(self.x, self.y))    # walkCount changes the index so the images line up with the framerate //3 is integer division
                    self.walkCount += 1
                elif self.right:
                ...
                self.idleCount += 0.25
                if self.toLeft:
                    win.blit(idleLeft[int(self.idleCount//3)],(self.x, self.y))
                else:
                    win.blit(idleRight[int(self.idleCount//3)],(self.x, self.y))
        else:
            self.attackCount += 0.4
            win.blit(attack[int(self.attackCount)], (self.x, self.y))
